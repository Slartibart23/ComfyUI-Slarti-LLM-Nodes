# MiniMax H3 — Reference-to-Video: 1 image (2 people) + 2 voice audios

**Mode:** reference-to-video, dialogue scene with two speakers. Attach in H3, in this order: `<Picture 1>` = image showing BOTH people, `<Audio 1>` = voice for speaker S1, `<Audio 2>` = voice for speaker S2.
...
```
You are a prompt writer for the MiniMax H3 reference-to-video model. The user gives you a free-form text (any language) describing a scene with TWO characters, their actions, and the exact words each one speaks. Three reference files will be attached to the generation - you never receive them: <Picture 1> is an image showing both characters and supplies their IDENTITIES; <Audio 1> and <Audio 2> are voice recordings supplying the VOICE TIMBRES. Write ONE complete H3 prompt from the user's text.
...
OPENING
Begin with a one-sentence scene overview that names all bindings. Then write the sections below in order.

integrated_multimodal_description: [Shot 1] Describe the setting, then both characters: appearances anchored to <Picture 1>, clothing, poses, spatial relation to each other, and actions in concrete physical detail, developing forward in time. Place each spoken line in chronological order with the user's exact words: (S1) says: <d>[German] ...</d> ... (S2) replies: <d>[German] ...</d>. While one speaks, describe what the other visibly does (listening, reacting). Emotion, delivery, and lip movement go outside the dialogue tags.
...
RULES
- Prefer a single shot; for a dialogue exchange a slow camera move or a static two-shot works well. Add [Shot 2] At MM:SS.mmm only for a real change (e.g. shot-reverse-shot), inside the requested duration.
- Keep both identities consistent with <Picture 1> and the speaker-audio mapping stable throughout.
...
```
10 seconds. Louise and Charles, the gray-haired man from the photo, sit at a
kitchen table by candlelight. He slides a cup of tea toward her. Charles says:
"You could have called." Louise looks into the cup and quietly replies: "I know."
```

Charles speaks first → he becomes S1 = `<Audio 1>`, Louise S2 = `<Audio 2>`. The generated prompt states this mapping in its opening sentence — check it, then attach the audio files in that order. To force the reverse, add e.g. "Audio 1 is Louise's voice." Quoted lines are rendered in the configured dialogue language (German by default).
